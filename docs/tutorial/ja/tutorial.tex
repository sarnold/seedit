...
\title{SELinux Policy Editor(seedit)管理ガイド（マニュアル) 2.1}
...
この文書は、SELinux Policy Editor 2.0のマニュアルです。基本概念や設定方法を紹介しています。
...
seeditは、Simplified Policy(単純化ポリシー)と、GUIなどSimplified Policy周辺ツー
ルから成り立つツールです。\\
...
Simplified Policyの、情報フロー分析可能性が証明されていません。
「ポリシーは、形式的（数学的）
に分析可能であるべきである」と考える人は、seeditを使ってはいけません。
SELinuxのstrictポリシーを頑張って使ってください。\\
Simplified Policyから生成されるSELinuxのポリシーを分析すれば、情報フロー
分析を可能かもしれません。ラベル付けのルールも、情報フロー分析に気をつかっ
てはいます。しかし、情報科学的に証明された物ではありません。
...
\section{GUIの概要}
seeditのGUIを使うことで、より簡単にSELinuxを使うことができます。
X Window Systemから、「seedit-gui」または「システム→設定→
SELinux Policy Editor」(Cent OSの場合は「アプリケーション」→「システム
設定」→「SELinux Policy Editor」)または、コンソールから「seedit-gui」とすることでGUIが起動
します。
すると、
...
\begin{itemize}
 ...
 \item ポリシーを生成(\ref{sec:addpolicy}節)
	...
	\item SELinuxのアクセスログからポリシーを生成
       \end{itemize}
 \item ポリシーを編集(\ref{sec:guieditor}節)
       \begin{itemize}
...
システムの状況を把握するために、 {\it ステータス} をコントロールパネルか
...
ます。\\
 {\it seedit
ドメイン・ロール管理}というウィンドウが開きます(図\ref{fig:manage-delete})。 {\it ドメインを削除}タブを選択します。

\begin{figure}
\caption{ドメイン管理ツール}\label{fig:manage-delete}
...
が挿入されます。
\begin{figure}
\caption{Simplified Policy用エディタ}\label{fig:edit}
\includegraphics*{images/edit.png}
\end{figure}

\begin{figure}
\caption{ファイルアクセス制御設定を追加}\label{fig:edit-file}
\includegraphics*{images/edit-file.png}
\end{figure}

\begin{figure}
\caption{ネットワークアクセス制御を追加}\label{fig:edit-network}
...
フルパス情報がログに含まれないことを意味します。auditdが動いてない場合は、locateコマンドを使ってフルパスを推測しますが、失敗することも多いです。
ですので、SELinux Policy
...
\subsection{ポリシ生成の諸注意}
\begin{itemize}
 \item chrootしているアプリケーションではフルパスがうまく生成されないこ
       とも\\
       例えば、vsftpdは、/var/ftpにchrootします。/var/ftp以下のファイル
       にアクセスし、ポリシ生成をした時、
       allow /pub r,s;のようなポリシが生成されますが、ファイル名が正しく
       ありません。実際は、/var/ftp/pubと記述しなくてはいけません。
...
\end{figure}
ちなみに、対象アプリをrpmパッケージでインストールしている場合は、「必要
なポリシを推測」ボタン使うことで、時間を節約できます。このボタンを押すと、
rpmパッケージの情報を参照することで、ある程度必要なポリシが生成されます。
今回の例の場合、
\begin{verbatim}
...
\begin{figure}
 \caption{ファイルアクセス制御の設定追加}\label{fig:add-file}
 \includegraphics*{images/add-file.png}
\end{figure}


\begin{figure}
\caption{ネットワークアクセス制御設定追加}\label{fig:add-network}
...
\section{質問をしたい}
質問があれば、気軽に 中村 himainu-ynakam@miomio.jp までメールを下さい。\\
匿名で質問をしたい場合は、SELinuxフォーラム(http://intrajp.no-ip.com/xoops/)を活用して下さい。
