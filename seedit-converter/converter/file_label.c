 ...
 */
/* $Id: file_label.c,v 1.11 2006/05/09 16:18:11 ynakam Exp $ */
...
void out_homedir_template(FILE *homedir_template){
#ifdef DIRSEARCH
  ...
  for (i = 0; i < file_label_table->element_num; i++){
	...
	if (strcmp(filename,"/")==0){
	  snprintf(fc_str, sizeof(fc_str), "HOME_DIR/(|.*)\tsystem_u:object_r:%s\n", fl->labelname);	
	}else{
	  snprintf(fc_str, sizeof(fc_str), "HOME_DIR%s(|/.*)\tsystem_u:object_r:%s\n", filename, fl->labelname);	
	}
      }else{
	if (strcmp(filename, "/") == 0){
	  snprintf(fc_str, sizeof(fc_str), "HOME_DIR%s(|[^/]*) \t system_u:object_r:%s\n", filename, fl->labelname);
    ...
    if(strcmp(fc_str, prev_fc_str) != 0){
      fprintf(homedir_template,"%s",fc_str);
    }    
    strncpy(prev_fc_str, fc_str, sizeof(fc_str));
  }
  
#ifdef DIRSEARCH
  fprintf(homedir_template, "#These labels are to support dir:search permission\n");
  ...
  for (i = 0; i<dir_label_table->element_num; i++){
    ...
    if(strcmp((fl->filename)+1,"/")==0){
      fprintf(homedir_template, "HOME_DIR\tsystem_u:object_r:%s\n",  fl->labelname);
    }else{
      fprintf(homedir_template, "HOME_DIR%s\tsystem_u:object_r:%s\n", (fl->filename)+1, fl->labelname);
    }
  }
  fprintf(homedir_template, "#End of dir:search\n");
  /*dummy HOME_ROOT*/
  fprintf(homedir_template, "HOME_ROOT <<none>>\n");
 ...
 *  @return:	none
 */
void out_file_context(FILE *file_contexts) {
  int i;
  FILE_LABEL *fl;
  struct stat buf;
  char fc_str[MAX_FILENAME]="";
  ...
  /*print file_contexts*/
  fprintf(file_contexts, "#This file is generated by seedit-converter\n");

  fprintf(file_contexts, "/(|.*)\t system_u:object_r:default_t\n");
  
  for (i = 0; i < file_label_table->element_num; i++){
    fl = (FILE_LABEL *)search_element(file_label_table, file_name_buf[i]);
    
    if(check_exist_in_list(file_name_buf[i],proc_mount_point_list)){
...
void out_file_contexts_config(FILE *file_contexts, FILE *homedir_template){
  ...
  out_file_context(file_contexts);
  out_homedir_template(homedir_template);
