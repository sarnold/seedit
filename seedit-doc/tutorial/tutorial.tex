...
\end{verbatim}
If you use Fedora Core 5, download seedit-converter-1.4.x.rpm and seedit-policy-2.0.x-FC5.rpm
 \item Install rpms
...
\begin{itemize}
 ...
 \item To upgrade from old version, you have to do 
	...
	# rm -rf /etc/selinux/seedit
       \end{verbatim}
 \item No GUI\\
       GUI is not under construction. However, we are sure our tool is much
       more usable than old version even without GUI.
\end{itemize}
...
policies are under construction :-) .

\subsection{seedit-unconfined}
Some processes are given domain and behavior is confined.
To know what is confined and what is unconfined, you can use
seedit-unconfined command. 
...
/root).
Note that if you write policy for individual user(such as allow
/home/ynakam/foo/** r;), it is neglected.\\
...
To generate permissoin o,a,c,e,t, use s option, like audit2spdl -dls .
...
%コンパイルエラーの見方
\\
SPDL has advanced feature to configure RBAC and more strict policy, it
is out of scope of document, it will be introduced later day.
...
\begin{enumerate}
 \item Give up controling access for individual files\\
       you can write\\
...
\end{verbatim}
       ...
       is given label(foo\_foo\_bar\_t) and identifined by label.
...
\end{verbatim}
       In default policy,allowtmp is used to control access to /etc/mtab and temporaly
...
\end{enumerate}
 \item Device files other than /dev\\
       ...
       directry, you can not access it.
