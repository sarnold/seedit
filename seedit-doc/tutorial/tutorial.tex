...
By {\it Add} button, you can insert policy at cursor point from GUI selection window.
...
\clearpage
\section{Add policy by policy generation tool}
If you find confined application run due to SELinux denial, you have to
add policy. 
If application run in permissive mode, but does not run in enforcing
mode, it is highly possible that SELinux is denying some access. You
have to add policy. We have GUI and command line utility.


\subsection{GUI(policy generator)}
...
\begin{itemize}
 \item Input\\
       ...
       dmesg. dmesg contains less information than audit.log, so
       accuracy of genarated policy is reduced. Therefore it is
       recommended to enabled auditd service and select audit.log.
